...
library(boot)
set.seed(1)
# Read the cv.glm(cross validation for generalized linear models) documentation
??cv.glm
...
# Therefore, the estimated test MSE for the linear regression fit is 26.14 10
...
#loading the dataset
data10<-read.csv(file.choose(),header=T)
head(data10)
#adding columns
colnames(data10)<-c("BuyingPrice","Maintenance","NumDoors","NumPersons","BootSpace","Safety","Condition")
head(data10)
str(data10)
#condition has 4 levels "acc","good","unacc","vgood"
levels(data10$Condition)
summary(data10)
#Creating "training dataset" and "Validation dataset"
#randomly choose 70% for training and 30% for Validation
set.seed(100)
help("sample")
train <- sample(nrow(data10),0.7*nrow(data10),replace=FALSE)
TrainSet <- data10[train,]
ValidSet <- data10[-train,]
summary(TrainSet)
summary(ValidSet)
